...
class PuzzleView(NamedTuple):
    board_metadata: BoardMetadataDTO
...
def get_puzzle_view(puzzle_number: int) -> PuzzleView:
    ...
    board_metadata = map_to_board_metadata_dto(board) # not serialized b/c used directly for django templates

    design_note = board.design_notes.order_by("design_number").first()
    return PuzzleView(board_metadata, serialized_board_view, serialized_solution_view, design_note, next_puzzle)
