...
def _serialize_cell(c: CellDTO) -> dict:
    return {
        ...
        "col": c.col,
        "placement_positions": {
            k.value: _serialize_placement_position(v)
...
def _serialize_letter(letter: LetterDTO) -> dict:
    return {"row": letter.row, "col": letter.col, "letter": letter.letter}


def _serialize_placement_position(p: PlacementPositionDTO) -> dict:
    return {"placement_id": p.placement_id, "placement_index": p.placement_index}
