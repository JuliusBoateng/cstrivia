...
def _map_to_board_dto(board: Board) -> BoardDTO:
    categories = [
        category.name for category in board.categories.all()
    ]  # categories prefetch prevents N+1 query
    return BoardDTO(
        board.id,
        board.title,
        board.author,
...
def _map_to_cell_dtos(placements: QuerySet[CluePlacement]) -> list[CellDTO]:
    ...
    for placement in placements:
        ...
        for c in placement.clue_cells.all():  # clue_cells prefetch prevents N+1 query
            ...
            placement_position = _map_to_placement_position(
                placement.id, c.placement_index
            )
...
def _map_to_placement_position(
    placement_id: int, placement_index: int
) -> PlacementPositionDTO:
